...
class LookCreate(BaseModel):
    ...
    age: int = Field(
        ...
        ge=10,
        le=100,
        description="Возраст пользователя (от 10 до 100 лет)"
    )
    
    style: Style = Field(
        ...,
        description="Предпочитаемый стиль одежды"
    )
...
class LookVariant(BaseModel):
    ...
    title: str = Field(
        ...
        min_length=1,
        max_length=100,
        description="Название варианта / стиля"
    )
...
class ValidationErrorDetail(BaseModel):
    ...
    field: str = Field(
        ...,
        description="Название поля, на котором произошла ошибка"
    )
...
class ApiResponseError(BaseModel):
    ...
    details: Optional[List[ValidationErrorDetail]] = Field(
        default=None,
        description="Детали ошибки (для валидационных ошибок)"
    )